...
def pairwise_interpolate_predictions(
    ...
    # Flatten 
    image_i_flat = image_i.flatten()
    distances_i_flat = distances_i.flatten()
...
#         point_from_j_nns = np.concatenate((uv_i[point_nn_indexes], image_i_flat[point_nn_indexes][..., None]), axis=1)
...
#                 interpolator = interpolate.interp2d(point_from_j_nns[:, 0], point_from_j_nns[:, 1], distances_i_flat[point_nn_indexes])
...
#                 interp_mask[idx] = False
    points_nn_indexes = nn_indexes_in_i
    points_from_j_nns = np.concatenate((uv_i[points_nn_indexes], image_i_flat[points_nn_indexes][..., None]), axis=-1)
    distances_to_nearest = np.sqrt(((reprojected_j[:, None, :] - points_from_j_nns)**2).sum(axis=-1))
    interp_mask = np.all(distances_to_nearest < distance_interpolation_threshold, axis=-1)
    
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        for idx in np.argwhere(interp_mask):
            try:
                point_nn_indexes = nn_indexes_in_i[idx]
                point_from_j = np.squeeze(reprojected_j[idx])
                point_from_j_nns = np.squeeze(points_from_j_nns[idx])
                # TODO: your code here: use `interpolate.interp2d`
                #  to construct a bilinear interpolator from distances predicted
                #  in `view_i` (i.e. `distances_i`) into the point in `view_j`.
                #  Use the interpolator to compute an interpolated distance value.
